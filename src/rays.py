...
def colour(rays, device, origins, objects):
    ...
    for i in range(5):
        ...
        if metalMask.any():
            fuzz = torch.where(metalMask.unsqueeze(-1), closestMaterial[..., 1:2], torch.zeros_like(closestMaterial[..., 1:2])) 
            metal = directions - (2* torch.sum((directions)*(closestNormals), dim=-1, keepdim=True) * closestNormals)
            metal = metal + (fuzz * torch.nn.functional.normalize(torch.randn_like(closestNormals), dim=-1))
        ...
        rays = directions  
        throughput = torch.where(hitMask.unsqueeze(-1), throughput * closestColours, throughput)
